{ config, pkgs, ... }: {
   services.sxhkd = {
	...
	# Reload bspwm config
	"super + shift + BackSpace" = "bspc wm -r";

	# Close or kill app
	#super + shift + k = bspc node {-c,-k}
	...
	"super + {h,v,r}" = "bspc node -p {east,south,cancel}";

	# Toggle beetwen floating & tiled
	"super + space" = "bspc node -t {floating,tiled}";

	# Pseudo Tiled & tiled mode
	"super + {p,t}" = "bspc node -t {pseudo_tiled,tiled}";
	...
	"super + shift + {1-6}" = "bspc node -d '^{1-6}' -f";
